...
let stalemate = false;
let isDrag = false;
let isPuzzle = parseInt(document.currentScript.getAttribute('isPuzzle'));
...
function drag(piece) {
	const drpiece = document.getElementById(piece);
	...
	drpiece.addEventListener("mousedown", function (e) {
		...
		// ✅ Allow only your pieces
		if (piece[0] !== window.myRole[0].toUpperCase()) return;

		// ✅ Wait until turn is loaded
		if (currentTurn === null) {
			console.log("Waiting for turn info...");
			return;
		}

		// ✅ Block if not your turn
		if (currentTurn !== window.myRole) {
...
document.getElementById("PromToBknight").addEventListener('click', function () { prom("B", "knight", 1, "d", "n"); });
...
document.getElementById("PromToBknight").addEventListener('touchstart', function () { prom("B", "knight", 1, "d", "n"); });
		...
		for (let i = 0; i < 64; i++) {
			if (squares[i].getBoundingClientRect().x <= x_final && squares[i].getBoundingClientRect().left + squares[i].clientWidth > x_final && squares[i].getBoundingClientRect().top <= y_final && squares[i].getBoundingClientRect().top + squares[i].clientHeight > y_final) {
				if (canGo(squares[i], true)) {
					...
					if (window.myRole && window.playerRef) {
						console.log(`[${window.myRole}] prevMove: ${fromsqr.id}, newMove: ${squares[i].id}`);

						window.playerRef.update({
							prevMove: fromsqr.id,
							newMove: squares[i].id
						});

						// ✅ Switch turn to the other player
						const nextTurn = window.myRole === "white" ? "black" : "white";
						firebase.database().ref("turn").set(nextTurn);
					}
...
firebase.database().ref("players").on("value", (snapshot) => {
	...
	for (let id in players) {
		...
		if (player.role === window.myRole) continue;

		const prev = player.prevMove;
		const next = player.newMove;

		// If there's a new opponent move and we haven't applied it yet
		const moveKey = `${prev}-${next}`;
		if (prev && next && moveKey !== lastOpponentMove) {
			lastOpponentMove = moveKey;

			console.log(`[opponent ${player.role}] moved from ${prev} to ${next}`);

			// Visually move the piece
			applyOpponentMove(prev, next);
		}
	}
});
...
function applyOpponentMove(fromId, toId) {
	const fromSquare = document.getElementById(fromId);
	const toSquare = document.getElementById(toId);

	if (!fromSquare || !toSquare) {
		console.warn("Invalid square IDs:", fromId, toId);
		return;
	}

	let pieceId = "";
	for (let i = 0; i < fromSquare.innerHTML.length; i++) {
		const ch = fromSquare.innerHTML[i];
		if (/[a-zA-Z0-9]/.test(ch)) pieceId += ch;
		else break;
	}
	...
	emptDiv.append(piece);

	piece.style.left = toSquare.getBoundingClientRect().left + "px";
	piece.style.top = toSquare.getBoundingClientRect().top + scrollY + "px";

	toSquare.innerHTML = pieceId;
